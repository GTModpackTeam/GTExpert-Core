...
public class EIORecipeLoader {
    public static void init() {
        ...
        RecipeMaps.ASSEMBLER_RECIPES.recipeBuilder().notConsumable(new IntCircuitIngredient(1)).input(MetaTileEntities.HULL[LV], 1).input(ModObject.itemSoulVial.getItemNN(), 2).input(Items.DIAMOND_SWORD, 3).input(SENSOR_LV,1).input(plateDouble, Steel, 2).output(VIAL_EXTRACTOR[1], 1).duration(40).EUt(VA[LV]).buildAndRegister();
